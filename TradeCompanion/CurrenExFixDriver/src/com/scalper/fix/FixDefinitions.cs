...
/// </summary>
using System;
using CfgReader = com.scalper.util.CfgReader;
using log4net;
...
namespace com.scalper.fix
{
	...
	public class FixDefinitions
	{
		...
		{
			lock (typeof(com.scalper.fix.FixDefinitions))
			{
		...
		public static FixDefinitions getFIXDefinition()
		{
			lock (typeof(com.scalper.fix.FixDefinitions))
			{
				if (FixDefinitions.fixDefinitions == null)
				{
					...
					FixDefinitions.getFIXDefinition(com.scalper.fix.FixConstants_Fields.beginString42, ref tempAux);
				}
				return FixDefinitions.fixDefinitions;
			}
		}
