...
namespace com.scalper.fix.driver.client
{
	public abstract class DefaultClient:AbstractClient
	{
		...
		{
			public AnonymousClassClientSessionCustomizer(com.scalper.fix.driver.FIXPersister persister, DefaultClient enclosingInstance)
			{
				InitBlock(persister, enclosingInstance);
			}
			private void  InitBlock(com.scalper.fix.driver.FIXPersister persister, DefaultClient enclosingInstance)
			{
				this.persister = persister;
				this.enclosingInstance = enclosingInstance;
			}
			private com.scalper.fix.driver.FIXPersister persister;
        ...
        protected abstract void handleMsgReceived(com.scalper.fix.Message msg);
        protected abstract void handleMsgSent(com.scalper.fix.Message msg);
        private class DefaultClientMessageListener : FIXMessageListener
        {
            ...
            public virtual void messageReceived(com.scalper.fix.Message msg)
            {
                 enclosingInstance.handleMsgReceived(msg);
            }
            /// <summary>notification that a message has been sent by the driver </summary>
            public virtual void messageSent(com.scalper.fix.Message msg)
            {
