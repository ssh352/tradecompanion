...
namespace com.scalper.fix.driver.client
{
	...
	public abstract class AbstractClient : Client
	{
		...
		public virtual void  sendMessage(Message msg)
		{
			...
			if (!setInitialValues)
			{
				System.String senderCompID = msg.getValue(com.scalper.fix.Constants_Fields.TAGSenderCompID_i);
				if (senderCompID != null)
				{
					session.SenderCompID = senderCompID;
				}
				
				System.String targetCompID = msg.getValue(com.scalper.fix.Constants_Fields.TAGTargetCompID_i);
		...
		{
			Message msg = new Message(com.scalper.fix.Constants_Fields.MSGHeartbeat);
		...
		public virtual Message accept(long timeout)
		{
			...
			// @todo
			if (message != null && !BasicFixUtilities.isAdmin(message) && message.getCharValue(com.scalper.fix.Constants_Fields.TAGPossDupFlag) != 'Y')
		...
		protected internal static bool isSequenceReset_Reset(Message msg)
		{
			...
			if (com.scalper.fix.Constants_Fields.MSGSequenceReset.Equals(msgType))
			{
				char gapFillFlag = msg.getCharValue(com.scalper.fix.Constants_Fields.TAGGapFillFlag_i);
