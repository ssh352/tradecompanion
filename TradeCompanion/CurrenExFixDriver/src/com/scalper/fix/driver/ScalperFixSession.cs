...
using BasicFixUtilities = com.scalper.fix.BasicFixUtilities;
using Constants = com.scalper.fix.Constants;
using FixConstants = com.scalper.fix.FixConstants;
using FixException = com.scalper.fix.FixException;
using FixMessageContentException = com.scalper.fix.FixMessageContentException;
using FixMessageFormatException = com.scalper.fix.FixMessageFormatException;
using Message = com.scalper.fix.Message;
using TagHelper = com.scalper.fix.TagHelper;
using BasicUtilities = com.scalper.util.BasicUtilities;
using BoundedQueue = com.scalper.util.BoundedQueue;
...
namespace com.scalper.fix.driver
{
    ...
    public class ScalperFixSession : FIXListenRegistry
    {
            ...
            set
            {
                ...
                if (OverwriteCompID)
                {
                    ...
                    if (senderCompID != null)
                        value.setValue(com.scalper.fix.Constants_Fields.TAGSenderCompID_i, senderCompID);
                    if (targetCompID != null)
                        value.setValue(com.scalper.fix.Constants_Fields.TAGTargetCompID_i, targetCompID);
                }
                else if (!testMode)
                //This is to provide facility in test mode to send the id's from def files.
                {
                    ...
                    if (senderCompID != null)
                        value.setIfNull(com.scalper.fix.Constants_Fields.TAGSenderCompID_i, senderCompID);
                    if (targetCompID != null)
                        value.setIfNull(com.scalper.fix.Constants_Fields.TAGTargetCompID_i, targetCompID);
                }
                ...
                if (value.Admin || SetSubIDOnApplicationMessages)
                {
                    ...
                    if (senderSubID != null)
                    {
                        value.setIfNull(com.scalper.fix.Constants_Fields.TAGSenderSubID_i, senderSubID);
                    }

                    // overwrite targetSubID if user wants it
                    if (targetSubID != null)
                    {
                        value.setIfNull(com.scalper.fix.Constants_Fields.TAGTargetSubID_i, targetSubID);
                    }
                }

                value.setIfNull(com.scalper.fix.Constants_Fields.TAGSenderLocationID_i, senderLocationID, false);
                value.setIfNull(com.scalper.fix.Constants_Fields.TAGTargetLocationID_i, targetLocationID, false);
            }
        ...
        {
            Message heartbeatMsg = createMessage(com.scalper.fix.Constants_Fields.MSGHeartbeat);

            // Add the TestReqID field
            if (testReqID != null)
                heartbeatMsg.setValue(com.scalper.fix.Constants_Fields.TAGTestReqID_i, testReqID);
        ...
        {
            Message testRequestMsg = createMessage(com.scalper.fix.Constants_Fields.MSGTestRequest);
            testRequestMsg.setValue(com.scalper.fix.Constants_Fields.TAGTestReqID_i, testReqID);
        ...
        {
            Message resendRequestMsg = createMessage(com.scalper.fix.Constants_Fields.MSGResendRequest);

            if (sessionState.otherSessionMustResend())
            {
                if (!properties.getBooleanProperty(SessionProperties.Prop_RequestMultipleResends))
                    return null;
                resendRequestMsg.setValue(com.scalper.fix.Constants_Fields.TAGPossResend, "Y");
            }

            sessionState.setOtherSessionMustResend();

            resendRequestMsg.setValue(com.scalper.fix.Constants_Fields.TAGBeginSeqNo_i, beginSeqNo);
            if (useInfinityInstead)
            {
                resendRequestMsg.setValue(com.scalper.fix.Constants_Fields.TAGEndSeqNo_i, resendInfinity);
            }
            else
            {
                resendRequestMsg.setValue(com.scalper.fix.Constants_Fields.TAGEndSeqNo_i, lastExpected);
            }
        ...
        private Message buildRejectMessage(Message badMsg, int code, System.String reason, int refTagID)
        {
            ...
            Message rejectMsg = createMessage(com.scalper.fix.Constants_Fields.MSGReject);

            rejectMsg.setValue(com.scalper.fix.Constants_Fields.TAGRefSeqNum_i, badMsgSeqNum);

            if (reason != null && !reason.Equals(""))
            {
                rejectMsg.setValue(com.scalper.fix.Constants_Fields.TAGText_i, reason);
            }
            ...
            if (minorVersion >= '2')
            {
                if (code != FixException.REJECT_REASON_NOT_SET)
                {
                    ...
                    {
                        rejectMsg.setValue(com.scalper.fix.Constants_Fields.TAGBusinessRejectReason_i, code);
                    }
                }
                if (refTagID > -1)
                {
                    rejectMsg.setValue(com.scalper.fix.Constants_Fields.TAGRefTagID_i, refTagID);
                }
                ...
                if (badMsgType != null && !badMsgType.Equals(""))
                    rejectMsg.setValue(com.scalper.fix.Constants_Fields.TAGRefMsgType_i, badMsgType);
            }
        ...
        {
            Message sequenceResetMsg = createMessage(com.scalper.fix.Constants_Fields.MSGSequenceReset);

            sequenceResetMsg.MsgSeqNum = msgSeqNum;
            sequenceResetMsg.setValue(com.scalper.fix.Constants_Fields.TAGGapFillFlag_i, 'Y');
            sequenceResetMsg.setValue(com.scalper.fix.Constants_Fields.TAGNewSeqNo_i, newSeqNo);
            sequenceResetMsg.setValue(com.scalper.fix.Constants_Fields.TAGPossDupFlag_i, 'Y'); //PoosDupFlag has to be set.
        ...
        {
            Message logoutMsg = createMessage(com.scalper.fix.Constants_Fields.MSGLogout);
        ...
        internal static void createMandatoryFields(Message msg)
        {
            ...
            if (msgType.Equals(com.scalper.fix.Constants_Fields.MSGHeartbeat))
            {
                // Do nothing
            }
            else if (msgType.Equals(com.scalper.fix.Constants_Fields.MSGTestRequest))
            {
                ...
                msg.addValue(com.scalper.fix.Constants_Fields.TAGTestReqID_i, "");
            }
            else if (msgType.Equals(com.scalper.fix.Constants_Fields.MSGResendRequest))
            {
                // if ResendRequest message
                msg.addValue(com.scalper.fix.Constants_Fields.TAGBeginSeqNo_i, 0);
                msg.addValue(com.scalper.fix.Constants_Fields.TAGEndSeqNo_i, 0);
            }
            else if (msgType.Equals(com.scalper.fix.Constants_Fields.MSGReject))
            {
                // if Reject message
                msg.addValue(com.scalper.fix.Constants_Fields.TAGRefSeqNum_i, 0);
            }
            else if (msgType.Equals(com.scalper.fix.Constants_Fields.MSGSequenceReset))
            {
                // if SequenceReset message
                msg.addValue(com.scalper.fix.Constants_Fields.TAGNewSeqNo_i, 0);
            }
            ...
            {
                // If Logon message
                msg.addValue(com.scalper.fix.Constants_Fields.TAGEncryptMethod_i, 0);
                msg.addValue(com.scalper.fix.Constants_Fields.TAGHeartBtInt_i, 0);
            }
        }
        ...
        public class FIXAdminProcessor : SupportClass.ThreadClass
        {
                    ...
                    switch (cmd.cmdType)
                    {
                        ...
                        case SendCommand.SEND:
                            ...
                            if (msg.Admin)
                            {
                                ...
                                if ((msgType != null) && msgType.Equals(com.scalper.fix.Constants_Fields.MSGHeartbeat))
                                {
                                    System.String testReqID = msg.getValue(com.scalper.fix.Constants_Fields.TAGTestReqID_i);
                                    checkIfResending = (testReqID != null);
                                }
                                Enclosing_Instance.transmitInternal(msg, false, checkIfResending, true);
                                if (msgType != null)
                                {
                                    if (msgType.Equals(com.scalper.fix.Constants_Fields.MSGReject))
                                    {
            ...
            protected internal virtual void processLogout(Message msg)
            {
                ...
                if (sessionStateInt != SessionState.LOGOUT_SENT)
                {
                    ...
                    //CurrenEx server has scheduled logout every day,Currenex server goes down for 5minutes 
                    //in this case currenex driver raise any event with text message scheduled logout.
                    System.String text = msg.getValue(com.scalper.fix.Constants_Fields.TAGText_i);
                    if (text.Equals("scheduled logout"))
                    {
                        //Enclosing_Instance.possiblyNotifyListeners(this, FIXNotice.FIX_DISCONNECTED, "scheduled logout");                      
                        Enclosing_Instance.stop(FIXNotice.FIX_DISCONNECTED, msg.getValue(Constants_Fields.TAGText));   
                    }
                    else
                    {
            ...
            protected internal virtual void processHeartBeat(Message msg)
            {
                ...
                {
                    System.String tag = msg.getValue(com.scalper.fix.Constants_Fields.TAGTestReqID_i);
                    if (tag != null && !tag.Equals("") && !com.scalper.fix.driver.ScalperFixSession.isPossDup(msg) && !com.scalper.fix.driver.ScalperFixSession.isPossResend(msg))
                    {
            ...
            protected internal virtual void processResendRequest(Message msg)
            {
                ...
                int startSeqNo = msg.getIntValue(com.scalper.fix.Constants_Fields.TAGBeginSeqNo_i);
                // the last sequence number in the resend request
                int endSeqNo = msg.getIntValue(com.scalper.fix.Constants_Fields.TAGEndSeqNo_i);
                ...
                else if (endSeqNo == com.scalper.fix.Constants_Fields.RESEND_INFINITY_40_41 || endSeqNo == com.scalper.fix.Constants_Fields.RESEND_INFINITY_42)
                {
            ...
            protected internal virtual bool logoutIfRejectResend(Message msg)
            {
                ...
                System.Int32 refSeqNum = msg.getIntegerValue(com.scalper.fix.Constants_Fields.TAGRefSeqNum_i);
...
//                if ((refSeqNum != null) && Enclosing_Instance.resendSeqs.Contains(refSeqNum))
        ...
        private class FIXReaderThread : SupportClass.ThreadClass
        {
            ...
            internal sbyte[] delimiterBytes = new sbyte[] { (sbyte)com.scalper.fix.FixConstants_Fields.delimiter };
                ...
                Run()
            {
                ...
                catch (DenialOfServiceException dos)
                {
                    ...
                    System.String patternString = "8=FIX[.]4[.].^9=([\\d]+)(^.*^)10=[\\d]{3}^";
                    patternString = patternString.Replace('^', com.scalper.fix.FixConstants_Fields.delimiter);
            ...
            private void printMessageReceived(Message msg)
            {
                if ((Enclosing_Instance.PrintFixBefore || Enclosing_Instance.PrintFixAfter) && (true))
                {
                    ...
                    buf.Append(",35=");
                    buf.Append(msg.getValue(com.scalper.fix.Constants_Fields.TAGMsgType_i));
                    buf.Append(" ] ");
            ...
            protected internal virtual void frameMessage(Message parsedMsg)
            {
                ...
                {
                    System.String text = parsedMsg.getValue(com.scalper.fix.Constants_Fields.TAGText_i);
                    if (text == null || text.Equals(""))
                    {
                        text = "<no message available>";
                    }
                    //This event riase two time need not riase this event two time
                    ...
                    // @todo should drop connection right away
                    //Enclosing_Instance.stop();
                    Enclosing_Instance.stop(FIXNotice.FIX_DISCONNECTED, parsedMsg.getValue(Constants_Fields.TAGText));
                    return;
                }
                ...
                if (sessionStateInt == SessionState.WAITING_FOR_LOGON || sessionStateInt == SessionState.LOGON_SENT)
                {
                    if (msgType != 'A')
                    {
                        ...
                        Message logoutMsg = Enclosing_Instance.buildLogoutMessage();
                        Enclosing_Instance.OptionalTags = logoutMsg;
                        logoutMsg.setValue(com.scalper.fix.Constants_Fields.TAGText_i, errorText);
                        //Send logout
                        Enclosing_Instance.transmitInternal(logoutMsg);
                        Enclosing_Instance.nextExpectedSeqNo++;
                        // @todo should drop connection right away
                ...
                if (seqNo == Enclosing_Instance.nextExpectedSeqNo)
                {
                    ...
                    if (BasicFixUtilities.isAdmin(parsedMsg))
                    {
                        ...
                        log.Info("framemessage if (BasicFixUtilities.isAdmin(parsedMsg))-> point 3" + DateTime.Now.ToString()); 
            ...
            protected internal virtual void processSequenceReset_GapFill(Message msg)
            {
                ...
                int newSeqNo = msg.getIntValue(com.scalper.fix.Constants_Fields.TAGNewSeqNo_i);
        ...
        public virtual void checkStoredMsg(Message msg)
        {
            ...
            char mVersion = msg.getValue(com.scalper.fix.Constants_Fields.TAGBeginString_i)[6];
        ...
        {
            if (msg.MsgType.Equals(com.scalper.fix.Constants_Fields.MSGReject))
            {
        ...
        private void transmitMsgSeqNumTooLow(Message msg)
        {
            ...
            try
            {
                ...
                logoutMsg.setValue(com.scalper.fix.Constants_Fields.TAGText_i, "MsgSeqNum too low, expecting " + nextExpectedSeqNo + " but received " + seqNo);
        ...
        {
            if (!ScalperFixSession.isAdmin(message.getCharValue(com.scalper.fix.Constants_Fields.TAGMsgType)))
                message.setValue(com.scalper.fix.Constants_Fields.TAGPossDupFlag_i, "Y");
            ...
            message.removeTag(com.scalper.fix.Constants_Fields.TAGBodyLength_i);
            message.removeTag(com.scalper.fix.Constants_Fields.TAGCheckSum_i);
        ...
        private void _transmitInternal(Message message, bool setOrigSendingTime, bool checkIfResending, bool messageIsFromDriver)
        {
            ...
            lock (transmitLock)
            {
                ...
                if (!setOrigSendingTime)
                {
                    ...
                    else if ((message.MsgType == null) || !message.MsgType.Equals(com.scalper.fix.Constants_Fields.MSGSequenceReset))
                    {
                        if (testMode)
                        {
        ...
        private void preprocessBeforeSend1(Message message, bool setOrigSendingTime, bool messageIsFromDriver)
        {
            ...
            if (PrintFixBefore)
            {
                ...
                buf.Append(",35=");
                buf.Append(message.getValue(com.scalper.fix.Constants_Fields.TAGMsgType_i));
                buf.Append(" ]");
            ...
            if (testMode)
            {
                message.setIfNull(com.scalper.fix.Constants_Fields.TAGBeginString_i, BeginFixVersion);
            }
            else
            {
                message.setValue(com.scalper.fix.Constants_Fields.TAGBeginString_i, BeginFixVersion);
            }
            ...
            else if (sessionStateInt != SessionState.WAITING_FOR_MESSAGE_SYNCH && sessionStateInt != SessionState.SESSION_READY && sessionStateInt != SessionState.LOGOUT_RECEIVED && sessionStateInt != SessionState.LOGON_RESP_SENT)
            {
                ...
                if (msgType != 'A' && !testMode)
                {
                    ...
                    else if (sessionStateInt == SessionState.LOGOUT_SENT && messageIsFromDriver && ((msgType == '4') || (message.getCharValue(com.scalper.fix.Constants_Fields.TAGPossDupFlag_i) == 'Y')))
                    {
                ...
                else if (msgType == 'A')
                {
                    ...
                    System.String beginString = message.getValue(com.scalper.fix.Constants_Fields.TAGBeginString_i);
                    if (beginString == null)
                    {
                        missing = true;
                    ...
                    if (!missing)
                    {
                        SenderCompID = message.getValue(com.scalper.fix.Constants_Fields.TAGSenderCompID_i);
                        if (senderCompID == null)
                        {
                            missing = true;
                    ...
                    if (!missing)
                    {
                        TargetCompID = message.getValue(com.scalper.fix.Constants_Fields.TAGTargetCompID_i);
                        if (targetCompID == null)
                        {
                            missing = true;
                    ...
                    if (!missing)
                    {
                        int hbInt = message.getIntValue(com.scalper.fix.Constants_Fields.TAGHeartBtInt_i);
                        if (hbInt == -1)
                        {
                            missing = true;
                    ...
                    else
                    {
                        ...
                        message.setValue(com.scalper.fix.Constants_Fields.TAGHeartBtInt_i, timer.HBeatInterval / 1000);
        ...
        private bool preprocessBeforeSend2(Message message, bool setOrigSendingTime, bool messageIsFromDriver)
        {
            ...
            if (!testMode || BasicUtilities.isNullOrEmpty(message.getSendingTime()) || setOrigSendingTime)
            {
                ...
                message.setValue(com.scalper.fix.Constants_Fields.TAGSendingTime_i, sendingTime);
            ...
            if (setOrigSendingTime)
            {
                ...
                message.setValue(com.scalper.fix.Constants_Fields.TAGOrigSendingTime_i, origSendingTime);
            }
            ...
            if (!message.containsTag(com.scalper.fix.Constants_Fields.TAGBodyLength_i))
                message.insertValueAt(com.scalper.fix.Constants_Fields.TAGBodyLength_i, "", 1);

            if (!message.containsTag(com.scalper.fix.Constants_Fields.TAGCheckSum_i))
                message.addValue(com.scalper.fix.Constants_Fields.TAGCheckSum, "");
        ...
        private void postprocessBeforeSend1(Message message)
        {
            ...
            */
            ...
            try
            {
                ...
                if (testMode && message.getCharValue(com.scalper.fix.FixConstants_Fields.TAGDropMessage) == 'Y' && !ScalperFixSession.isPossDup(message))
                {
        ...
        /// <throws>  FixException </throws>
        public virtual void stop(int _FIXNOTICE, string reason)
        {
            // to find out where we were called from:
            int sessionStateInt = sessionState.State;
            if (sessionStateInt != SessionState.SESSION_CLOSING && sessionStateInt != SessionState.SESSION_CLOSED)
            {
                stopInboundImpl(_FIXNOTICE, reason);
            }
        }

        public virtual void stop()
        {
            stop(FIXNotice.FIX_DISCONNECTED, "Disconnected");
        }
        ...
        protected internal virtual void stopInboundImpl(int _FIXNOTICE, string reason)
        {
            //Set the session state.
            ...
            State = SessionState.SESSION_CLOSED;
            //possiblyNotifyListeners(this, FIXNotice.FIX_DISCONNECTED, "Disconnected");
            possiblyNotifyListeners(this, _FIXNOTICE, reason);
        ...
        private void doValidate(Message message)
        {
            ...
            System.String sender = message.getValue(com.scalper.fix.Constants_Fields.TAGSenderCompID_i);
            if (sender == null)
            {
                ...
                fMfe.RefTagID = com.scalper.fix.Constants_Fields.TAGSenderCompID_i;
                throw fMfe;
            }
            ...
            System.String target = message.getValue(com.scalper.fix.Constants_Fields.TAGTargetCompID_i);
            if (target == null)
            {
                ...
                fMfe.RefTagID = com.scalper.fix.Constants_Fields.TAGTargetCompID_i;
                throw fMfe;
            }
        ...
        private void checkFieldsForMsgType(Message message)
        {
            ...
            if (msgType.Equals(com.scalper.fix.Constants_Fields.MSGLogon))
            {
                // Logon message
                ...
                try
                {
                    System.String encryptMethodStr = message.getValue(com.scalper.fix.Constants_Fields.TAGEncryptMethod_i);
                    //int encryptMethod=0;
                    ...
                    System.String heartBtIntStr = message.getValue(com.scalper.fix.Constants_Fields.TAGHeartBtInt_i);
            ...
            else if (msgType.Equals(com.scalper.fix.Constants_Fields.MSGTestRequest))
            {
                ...
                try
                {
                    System.String testReqID = message.getValue(com.scalper.fix.Constants_Fields.TAGTestReqID_i);
                    if (testReqID == null)
            ...
            else if (msgType.Equals(com.scalper.fix.Constants_Fields.MSGResendRequest))
            {
                ...
                try
                {
                    System.String beginSeqNoStr = message.getValue(com.scalper.fix.Constants_Fields.TAGBeginSeqNo_i);
                    if (beginSeqNoStr != null)
                        System.Int32.Parse(beginSeqNoStr);
                    else
                        throw new System.NullReferenceException("BeginSeqNo field is missing.");

                    System.String endSeqNoStr = message.getValue(com.scalper.fix.Constants_Fields.TAGEndSeqNo_i);
                    if (endSeqNoStr != null)
            ...
            else if (msgType.Equals(com.scalper.fix.Constants_Fields.MSGReject))
            {
                ...
                try
                {
                    System.String refSeqNumStr = message.getValue(com.scalper.fix.Constants_Fields.TAGRefSeqNum_i);
            ...
            else if (msgType.Equals(com.scalper.fix.Constants_Fields.MSGSequenceReset))
            {
                ...
                try
                {
                    System.String newSeqNoStr = message.getValue(com.scalper.fix.Constants_Fields.TAGNewSeqNo_i);
                    if (newSeqNoStr != null)
        ...
        private void setFixVersion(System.String newFixVersion)
        {
            ...
            switch (minorVersionTemp)
            {
                ...
                case '2': tagHelper = new TagHelper("FIX.4.2"); break;
            ...
            if (minorVersion >= '2')
            {
                resendInfinity = com.scalper.fix.Constants_Fields.RESEND_INFINITY_42;
            }
            else
            {
                resendInfinity = com.scalper.fix.Constants_Fields.RESEND_INFINITY_40_41;
            }
        }
        ...
        {
            char msgType = msg.getCharValue(com.scalper.fix.Constants_Fields.TAGMsgType_i);
            char gapFillChar = msg.getCharValue(com.scalper.fix.Constants_Fields.TAGGapFillFlag_i);
        ...
        {
            char possDupeFlag = msg.getCharValue(com.scalper.fix.Constants_Fields.TAGPossDupFlag_i);
            return possDupeFlag == 'Y';
        }
        private static bool isPossResend(Message msg)
        {
            char possResendFlag = msg.getCharValue(com.scalper.fix.Constants_Fields.TAGPossResend_i);
        ...
        public class SendCommand
        {
            private void InitBlock(ScalperFixSession enclosingInstance)
            {
                ...
                cmdType = com.scalper.fix.driver.ScalperFixSession.SendCommand.SEND;
            }
            private ScalperFixSession enclosingInstance;
            public ScalperFixSession Enclosing_Instance
            {
