...
using FIXByteBuffer = com.scalper.fix.FIXByteBuffer;
...
namespace com.scalper.fix.driver
{
	...
	public class DefaultMessageConnection:AbstractMessageConnection
	{
		...
		private static readonly int maxDigits = com.scalper.fix.FixConstants_Fields.MAX_BODY_LENGTH_DIGITS;
		
		private static readonly int START_READ_LENGTH = "8=FIX.4.X^9=^".Length + com.scalper.fix.FixConstants_Fields.MAX_BODY_LENGTH_DIGITS;
		...
		public override void  sendMessage(Message msg)
		{
			...
			conn.send(com.scalper.util.ByteBuffer.wrap(buf.array()));
		}
		...
		public override char getFixMinorVersion(System.Object tmpMessage)
		{
			...
			return (char) msgByte[com.scalper.fix.driver.MessageConnection_Fields.FIX_MINOR_VERSION_INDEX];
		}
		...
		private sbyte[] readInternal()
		{
			...
			int totalSize = bodyLength + startBodyIndex + com.scalper.fix.FixConstants_Fields.END_FIXMSG_SIZE;
		...
		{
			if (buf[workingCopyCurIndex++] != com.scalper.fix.FixConstants_Fields.delimiter)
			{
		...
		private int readBodyTag(sbyte[] buf, int maxDigits, System.String tagName)
		{
			...
			while (i <= maxDigits)
			{
				...
				if (ch == com.scalper.fix.FixConstants_Fields.delimiter)
				{
					workingCopyCurIndex--;
					break;
				}
			...
			if (length == 0 || (ch != com.scalper.fix.FixConstants_Fields.delimiter))
			{
		...
		public override void  getParsedMessage(Message blankMessage, System.Object unparsedMessageObj, TagHelper tagHelper, bool strictReceiveMode)
		{
			...
			while (index < msgLen)
			{
				...
				if (ch != com.scalper.fix.Constants_Fields.tagValueSeparatorChar)
				{
					...
					if (fe == null)
						fe = new FixMessageFormatException("Invalid tag number", FixException.REJECT_REASON_INVALID_TAG_NUM);
					// skip this tag/value
					while (index < msgLen && (char) unparsedMessage[index] != com.scalper.fix.Constants_Fields.delimiter)
						index++;
					if (index < msgLen)
						index++;
					continue;
				...
				while (index < msgLen && ((lenToRead != - 1 && ((index - startValueIndex) < lenToRead)) || unparsedMessage[index] != com.scalper.fix.Constants_Fields.delimiter))
				{
					index++;
				}
				...
				else
				{
					...
					{
						if (tag == com.scalper.fix.Constants_Fields.TAGSignature_i || tag == com.scalper.fix.Constants_Fields.TAGSecureData_i)
					...
					else
					{
						try
						{
							...
							message.addValue(tag, System.Text.Encoding.GetEncoding(com.scalper.fix.Constants_Fields.CHARSET_USED).GetString(SupportClass.ToByteArray(bytes)));
						}
					...
					if (tag == com.scalper.fix.Constants_Fields.TAGSignatureLength_i || tag == com.scalper.fix.Constants_Fields.TAGSecureDataLen_i)
					{
