...
namespace  Dukascopy.com.scalper.fix.driver
{
	...
	class FIXSessionTimer
	{
		...
		private class ScheduleItem
		{
			...
			{
                return ((delay > 0) && (elapsed_Renamed_Field + Dukascopy.com.scalper.fix.driver.FIXSessionTimer.TIMER_ALLOWED_OFFSET) >= delay);
			}
		}
		...
		public class TaskScheduler:SupportClass.ThreadClass
		{
			...
			override public void  Run()
			{
				while (keepRunning)
				{
					...
					try
					{
                        System.Threading.Thread.Sleep(new System.TimeSpan((System.Int64)10000 * Dukascopy.com.scalper.fix.driver.FIXSessionTimer.TIMER_GRANULARITY));
					}
					catch (System.Exception e)
					{
                        log.Error(e);
