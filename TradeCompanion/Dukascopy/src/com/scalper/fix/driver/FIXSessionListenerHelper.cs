...
//using Logging = Dukascopy.com.scalper.util.Logging;
using log4net;
namespace  Dukascopy.com.scalper.fix.driver
{
	...
	class FIXSessionListenerHelper
	{
		...
		private static System.String getText(Message msg)
		{
			if (msg == null)
				return null;
            return msg.getValue(Dukascopy.com.scalper.fix.Constants_Fields.TAGText_i);
		}
						...
						if (holder.sent)
						{
							while (it.MoveNext())
							{
								try
								{
                                    ...
                                    //VM_Fix listener.messageSent(msg);
								}
