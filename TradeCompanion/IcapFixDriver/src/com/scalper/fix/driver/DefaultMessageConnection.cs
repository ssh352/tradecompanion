...
//using FIXByteBuffer = Icap.com.scalper.fix.FIXByteBuffer;
...
namespace Icap.com.scalper.fix.driver
{
	...
	public class DefaultMessageConnection:AbstractMessageConnection
	{
		...
		private static readonly int maxDigits = Icap.com.scalper.fix.FixConstants_Fields.MAX_BODY_LENGTH_DIGITS;
		
		private static readonly int START_READ_LENGTH = "8=FIXT.1.X^9=^".Length + Icap.com.scalper.fix.FixConstants_Fields.MAX_BODY_LENGTH_DIGITS;
		...
		public override void  sendMessage(Message msg)
		{
			...
			conn.send(Icap.com.scalper.util.ByteBuffer.wrap(buf.array()));
		}
		...
		public override char getFixMinorVersion(System.Object tmpMessage)
		{
			...
			return (char) msgByte[Icap.com.scalper.fix.driver.MessageConnection_Fields.FIX_MINOR_VERSION_INDEX];
		}
		...
		private sbyte[] readInternal()
		{
			...
			match(initialBuffer, "FIXT.1.", "BeginString value doesn't starts with 'FIXT.1.'");//giriIcap
			...
			int totalSize = bodyLength + startBodyIndex + Icap.com.scalper.fix.FixConstants_Fields.END_FIXMSG_SIZE;
		...
		{
			if (buf[workingCopyCurIndex++] != Icap.com.scalper.fix.FixConstants_Fields.delimiter)
			{
		...
		private int readBodyTag(sbyte[] buf, int maxDigits, System.String tagName)
		{
			...
			while (i <= maxDigits)
			{
				...
				if (ch == Icap.com.scalper.fix.FixConstants_Fields.delimiter)
				{
					workingCopyCurIndex--;
					break;
				}
			...
			if (length == 0 || (ch != Icap.com.scalper.fix.FixConstants_Fields.delimiter))
			{
		...
		public override void  getParsedMessage(Message blankMessage, System.Object unparsedMessageObj, TagHelper tagHelper, bool strictReceiveMode)
		{
			...
			while (index < msgLen)
			{
				...
				if (ch != Icap.com.scalper.fix.Constants_Fields.tagValueSeparatorChar)
				{
					...
					if (fe == null)
						fe = new FixMessageFormatException("Invalid tag number", FixException.REJECT_REASON_INVALID_TAG_NUM);
					// skip this tag/value
					while (index < msgLen && (char) unparsedMessage[index] != Icap.com.scalper.fix.Constants_Fields.delimiter)
						index++;
					if (index < msgLen)
						index++;
					continue;
				...
				while (index < msgLen && ((lenToRead != - 1 && ((index - startValueIndex) < lenToRead)) || unparsedMessage[index] != Icap.com.scalper.fix.Constants_Fields.delimiter))
				{
					index++;
				}
				...
				else
				{
					...
					{
						if (tag == Icap.com.scalper.fix.Constants_Fields.TAGSignature_i || tag == Icap.com.scalper.fix.Constants_Fields.TAGSecureData_i)
					...
					else
					{
						try
						{
							...
							message.addValue(tag, System.Text.Encoding.GetEncoding(Icap.com.scalper.fix.Constants_Fields.CHARSET_USED).GetString(SupportClass.ToByteArray(bytes)));
						}
					...
					if (tag == Icap.com.scalper.fix.Constants_Fields.TAGSignatureLength_i || tag == Icap.com.scalper.fix.Constants_Fields.TAGSecureDataLen_i)
					{
