...
namespace Icap.com.scalper.fix.driver
{
	...
	class FIXSessionTimer
	{
		...
		public virtual void  stop()
		{
            ...
            //System.Console.Out.WriteLine((System.DateTime.Now.Ticks - 621355968000000000) / 10000 + ":Stopping the scheduler..");
			scheduler.cancel();
		}
		...
		private class ScheduleItem
		{
			...
			{
				return ((delay > 0) && (elapsed_Renamed_Field + Icap.com.scalper.fix.driver.FIXSessionTimer.TIMER_ALLOWED_OFFSET) >= delay);
			}
		}
		...
		public class TaskScheduler:SupportClass.ThreadClass
		{
			...
			override public void  Run()
			{
				while (keepRunning)
				{
					...
					try
					{
						System.Threading.Thread.Sleep(new System.TimeSpan((System.Int64) 10000 * Icap.com.scalper.fix.driver.FIXSessionTimer.TIMER_GRANULARITY));
					}
					catch (System.Exception e)
					{
                        log.Error(e);
		...
		public virtual void  resetWaitingForResend()
		{
			...
			lock (waitForResendRequestLock)
			{
				if (waitForResendRequestTask == null)
				{
					//System.Console.Out.WriteLine("Task to wait for resend was stopped, not starting...");
			...
			public override void  takeAction()
			{
				if (testRequestCount < Enclosing_Instance.TEST_REQUEST_BAILOUT - 1)
				{
					...
					catch //(System.Exception e)
					{
						...
						//System.Console.Out.WriteLine("FIXSessionTimer.takeTick: Failed transmitting test request");
					}
					testRequestCount++;
				...
				else
				{
					...
					Enclosing_Instance.session.possiblyNotifyListeners(this, FIXTNotice.FIX_TESTREQUEST_BAILOUT, "Counter-party is dead");
