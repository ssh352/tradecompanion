...
namespace Icap.com.scalper.fix.driver.client
{
	public abstract class DefaultClient:AbstractClient
	{
		...
		{
			public AnonymousClassClientSessionCustomizer(Icap.com.scalper.fix.driver.FIXPersister persister, DefaultClient enclosingInstance)
			{
				InitBlock(persister, enclosingInstance);
			}
			private void  InitBlock(Icap.com.scalper.fix.driver.FIXPersister persister, DefaultClient enclosingInstance)
			{
				this.persister = persister;
				this.enclosingInstance = enclosingInstance;
			}
			private Icap.com.scalper.fix.driver.FIXPersister persister;
		...
		private void init() 
        {
            HeartBeatInterval = 1; //15 sec//giriIcap
            //Configure logging (using Log4Net package in this software).
            XmlConfigurator.Configure(new System.IO.FileInfo(Environment.CurrentDirectory + "\\logging.xml"));
		...
		protected internal static System.String buildDateString(System.String timeString, bool useMillis)
		{
            ...
            {
                System.DateTime tempAux = DateTime.Now;
        ...
        protected abstract void handleMsgReceived(Icap.com.scalper.fix.Message msg);
        protected abstract void handleMsgSent(Icap.com.scalper.fix.Message msg);
        private class DefaultClientMessageListener : FIXMessageListener
        {
            ...
            public virtual void messageReceived(Icap.com.scalper.fix.Message msg)
            {
                 enclosingInstance.handleMsgReceived(msg);
            }
            /// <summary>notification that a message has been sent by the driver </summary>
            public virtual void messageSent(Icap.com.scalper.fix.Message msg)
            {
