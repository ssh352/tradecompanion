...
using BasicFixUtilities = Icap.com.scalper.fix.BasicFixUtilities;
using Constants = Icap.com.scalper.fix.Constants;
using FixConstants = Icap.com.scalper.fix.FixConstants;
using FixException = Icap.com.scalper.fix.FixException;
using FixMessageContentException = Icap.com.scalper.fix.FixMessageContentException;
using FixMessageFormatException = Icap.com.scalper.fix.FixMessageFormatException;
using Message = Icap.com.scalper.fix.Message;
using TagHelper = Icap.com.scalper.fix.TagHelper;
using BasicUtilities = Icap.com.scalper.util.BasicUtilities;
using BoundedQueue = Icap.com.scalper.util.BoundedQueue;
...
namespace Icap.com.scalper.fix.driver
{
    ...
    public class ScalperFixSession : FIXListenRegistry
    {
            ...
            set
            {
                ...
                if (OverwriteCompID)
                {
                    ...
                    if (senderCompID != null)
                        value.setValue(Icap.com.scalper.fix.Constants_Fields.TAGSenderCompID_i, senderCompID);
                    if (targetCompID != null)
                        value.setValue(Icap.com.scalper.fix.Constants_Fields.TAGTargetCompID_i, targetCompID);
                }
                else if (!testMode)
                //This is to provide facility in test mode to send the id's from def files.
                {
                    ...
                    if (senderCompID != null)
                        value.setIfNull(Icap.com.scalper.fix.Constants_Fields.TAGSenderCompID_i, senderCompID);
                    if (targetCompID != null)
                        value.setIfNull(Icap.com.scalper.fix.Constants_Fields.TAGTargetCompID_i, targetCompID);
                }
                ...
                if (value.Admin || SetSubIDOnApplicationMessages)
                {
                    ...
                    if (senderSubID != null)
                    {
                        value.setIfNull(Icap.com.scalper.fix.Constants_Fields.TAGSenderSubID_i, senderSubID);
                    }

                    // overwrite targetSubID if user wants it
                    if (targetSubID != null)
                    {
                        value.setIfNull(Icap.com.scalper.fix.Constants_Fields.TAGTargetSubID_i, targetSubID);
                    }
                }

                value.setIfNull(Icap.com.scalper.fix.Constants_Fields.TAGSenderLocationID_i, senderLocationID, false);
                value.setIfNull(Icap.com.scalper.fix.Constants_Fields.TAGTargetLocationID_i, targetLocationID, false);
            }
        ...
        {
            Message heartbeatMsg = createMessage(Icap.com.scalper.fix.Constants_Fields.MSGHeartbeat);

            // Add the TestReqID field
            if (testReqID != null)
                heartbeatMsg.setValue(Icap.com.scalper.fix.Constants_Fields.TAGTestReqID_i, testReqID);
        ...
        {
            Message testRequestMsg = createMessage(Icap.com.scalper.fix.Constants_Fields.MSGTestRequest);
            testRequestMsg.setValue(Icap.com.scalper.fix.Constants_Fields.TAGTestReqID_i, testReqID);
        ...
        {
            Message resendRequestMsg = createMessage(Icap.com.scalper.fix.Constants_Fields.MSGResendRequest);

            if (sessionState.otherSessionMustResend())
            {
                if (!properties.getBooleanProperty(SessionProperties.Prop_RequestMultipleResends))
                    return null;
                resendRequestMsg.setValue(Icap.com.scalper.fix.Constants_Fields.TAGPossResend, "Y");
            }

            sessionState.setOtherSessionMustResend();

            resendRequestMsg.setValue(Icap.com.scalper.fix.Constants_Fields.TAGBeginSeqNo_i, beginSeqNo);
            if (useInfinityInstead)
            {
                resendRequestMsg.setValue(Icap.com.scalper.fix.Constants_Fields.TAGEndSeqNo_i, resendInfinity);
            }
            else
            {
                resendRequestMsg.setValue(Icap.com.scalper.fix.Constants_Fields.TAGEndSeqNo_i, lastExpected);
            }
        ...
        private Message buildRejectMessage(Message badMsg, int code, System.String reason, int refTagID)
        {
            ...
            Message rejectMsg = createMessage(Icap.com.scalper.fix.Constants_Fields.MSGReject);

            rejectMsg.setValue(Icap.com.scalper.fix.Constants_Fields.TAGRefSeqNum_i, badMsgSeqNum);

            if (reason != null && !reason.Equals(""))
            {
                rejectMsg.setValue(Icap.com.scalper.fix.Constants_Fields.TAGText_i, reason);
            }
            ...
            if (minorVersion >= '2')
            {
                if (code != FixException.REJECT_REASON_NOT_SET)
                {
                    ...
                    {
                        rejectMsg.setValue(Icap.com.scalper.fix.Constants_Fields.TAGBusinessRejectReason_i, code);
                    }
                }
                if (refTagID > -1)
                {
                    rejectMsg.setValue(Icap.com.scalper.fix.Constants_Fields.TAGRefTagID_i, refTagID);
                }
                ...
                if (badMsgType != null && !badMsgType.Equals(""))
                    rejectMsg.setValue(Icap.com.scalper.fix.Constants_Fields.TAGRefMsgType_i, badMsgType);
            }
        ...
        {
            Message sequenceResetMsg = createMessage(Icap.com.scalper.fix.Constants_Fields.MSGSequenceReset);

            sequenceResetMsg.MsgSeqNum = msgSeqNum;
            sequenceResetMsg.setValue(Icap.com.scalper.fix.Constants_Fields.TAGGapFillFlag_i, 'Y');
            sequenceResetMsg.setValue(Icap.com.scalper.fix.Constants_Fields.TAGNewSeqNo_i, newSeqNo);
            sequenceResetMsg.setValue(Icap.com.scalper.fix.Constants_Fields.TAGPossDupFlag_i, 'Y'); //PoosDupFlag has to be set.
        ...
        {
            Message logoutMsg = createMessage(Icap.com.scalper.fix.Constants_Fields.MSGLogout);
        ...
        internal static void createMandatoryFields(Message msg)
        {
            ...
            if (msgType.Equals(Icap.com.scalper.fix.Constants_Fields.MSGHeartbeat))
            {
                // Do nothing
            }
            else if (msgType.Equals(Icap.com.scalper.fix.Constants_Fields.MSGTestRequest))
            {
                ...
                msg.addValue(Icap.com.scalper.fix.Constants_Fields.TAGTestReqID_i, "");
            }
            else if (msgType.Equals(Icap.com.scalper.fix.Constants_Fields.MSGResendRequest))
            {
                // if ResendRequest message
                msg.addValue(Icap.com.scalper.fix.Constants_Fields.TAGBeginSeqNo_i, 0);
                msg.addValue(Icap.com.scalper.fix.Constants_Fields.TAGEndSeqNo_i, 0);
            }
            else if (msgType.Equals(Icap.com.scalper.fix.Constants_Fields.MSGReject))
            {
                // if Reject message
                msg.addValue(Icap.com.scalper.fix.Constants_Fields.TAGRefSeqNum_i, 0);
            }
            else if (msgType.Equals(Icap.com.scalper.fix.Constants_Fields.MSGSequenceReset))
            {
                // if SequenceReset message
                msg.addValue(Icap.com.scalper.fix.Constants_Fields.TAGNewSeqNo_i, 0);
            }
            ...
            {
                // If Logon message
                msg.addValue(Icap.com.scalper.fix.Constants_Fields.TAGEncryptMethod_i, 0);
                msg.addValue(Icap.com.scalper.fix.Constants_Fields.TAGHeartBtInt_i, 0);
            }
        }
        ...
        internal virtual void readyToSendAppMessages()
        {
            ...
            lock (sessionStateLock)
            {
                ...
                possiblyNotifyListeners(this, FIXTNotice.FIX_SESSION_ESTABLISHED, "FIX session established");
            }
        }
        ...
        protected internal virtual bool possiblyNotifyListeners(System.Object source, int noticeCode, System.String noticeText, Message msg)
        {
            ...
            {
                FIXTNotice notice = new FIXTNotice(source, msg, this, noticeCode, noticeText);
        ...
        public class FIXAdminProcessor : SupportClass.ThreadClass
        {
                    ...
                    switch (cmd.cmdType)
                    {
                        ...
                        case SendCommand.SEND:
                            ...
                            if (msg.Admin)
                            {
                                ...
                                if ((msgType != null) && msgType.Equals(Icap.com.scalper.fix.Constants_Fields.MSGHeartbeat))
                                {
                                    System.String testReqID = msg.getValue(Icap.com.scalper.fix.Constants_Fields.TAGTestReqID_i);
                                    checkIfResending = (testReqID != null);
                                }
                                Enclosing_Instance.transmitInternal(msg, false, checkIfResending, true);
                                if (msgType != null)
                                {
                                    if (msgType.Equals(Icap.com.scalper.fix.Constants_Fields.MSGReject))
                                    {
                                        Enclosing_Instance.possiblyNotifyListeners(this, FIXTNotice.FIX_MESSAGE_REJECTED, "Message [" + cmd.origMsg + "] rejected", cmd.origMsg);
                                    }
                                }
                            }
                            else
                ...
                lock (Enclosing_Instance.transmitLock)
                {
                    ...
                    while (messagesToResend.MoveNext())
                    {
                        ...
                        if (resendCounter.getResendCount(currentSeqNum) > Enclosing_Instance.maxSendRetryCount)
                        {
                            ...
                            Enclosing_Instance.possiblyNotifyListeners(this, FIXTNotice.FIX_RESEND_BAILOUT, "Failed to resend message,Sending gapfill");
                            continue;
                        }
            ...
            protected internal virtual void processLogout(Message msg)
            {
                ...
                if (sessionStateInt != SessionState.LOGOUT_SENT)
                {
                    ...
                    Enclosing_Instance.possiblyNotifyListeners(this, FIXTNotice.FIX_LOGOUT_RECEIVED, "Received Logout", msg);
                ...
                else
                {
                    ...
                    Enclosing_Instance.possiblyNotifyListeners(this, FIXTNotice.FIX_LOGOUT_RESP_RECEIVED, "Received Logout response.", msg);
            ...
            protected internal virtual void processLogon(Message msg)
            {
                if (!Enclosing_Instance.logonSuccess)
                {
                        ...
                        Enclosing_Instance.possiblyNotifyListeners(this, FIXTNotice.FIX_LOGON_RESP_RECEIVED, "Logon response received", msg.Copy);
                        //We always do post processing of logon response.
                        processPostLogon(msg);
                        //Enclosing_Instance.State = SessionState.SESSION_READY; //GiriICap
                }
                else
                {
                    if (!Enclosing_Instance.RejectDupLogon)
                    {
                        if ((Enclosing_Instance.listener != null) && Enclosing_Instance.listenerInterests[FIXTNotice.FIX_DUP_LOGON])
                        {
                            FIXTNotice logonNotice = new FIXTNotice(this, msg, Enclosing_Instance, FIXTNotice.FIX_DUP_LOGON, "Duplicate Logon");
            ...
            protected internal virtual void processHeartBeat(Message msg)
            {
                ...
                {
                    System.String tag = msg.getValue(Icap.com.scalper.fix.Constants_Fields.TAGTestReqID_i);
                    if (tag != null && !tag.Equals("") && !Icap.com.scalper.fix.driver.ScalperFixSession.isPossDup(msg) && !Icap.com.scalper.fix.driver.ScalperFixSession.isPossResend(msg))
                    {
            ...
            protected internal virtual void processResendRequest(Message msg)
            {
                ...
                int startSeqNo = msg.getIntValue(Icap.com.scalper.fix.Constants_Fields.TAGBeginSeqNo_i);
                // the last sequence number in the resend request
                int endSeqNo = msg.getIntValue(Icap.com.scalper.fix.Constants_Fields.TAGEndSeqNo_i);
                ...
                else if (endSeqNo == Icap.com.scalper.fix.Constants_Fields.RESEND_INFINITY_40_41 || endSeqNo == Icap.com.scalper.fix.Constants_Fields.RESEND_INFINITY_42)
                {
            ...
            //Giri
            protected internal virtual void processUserResponse(Message msg)
            {
                string fValue = msg.getStringFieldValue(Constants_Fields.TAGUserStatus);
                string userText = "";
                switch (fValue)
                {
                    case "1":                        
                        Enclosing_Instance.possiblyNotifyListeners(this, FIXTNotice.FIX_APP_LOGON_SUCCESS, "Application login Successful", msg);
                        break;
                    case "2":
                        userText = msg.getStringFieldValue(Constants_Fields.TAGUserStatusText);
                        if(userText != "Clinet Intiated signoff")
                        {
                            Enclosing_Instance.possiblyNotifyListeners(this, FIXTNotice.FIX_APP_LOGON_FAILS, userText, msg);
                        }
                        break;
                    case "5":
                        Enclosing_Instance.possiblyNotifyListeners(this, FIXTNotice.FIX_PASS_CHANGED, "Password changed successfully", msg);
                        break;
                    case "1000":
                        userText = msg.getStringFieldValue(Constants_Fields.TAGUserStatusText);
                        Enclosing_Instance.possiblyNotifyListeners(this, FIXTNotice.FIX_PASS_EXPIRED, userText, msg);
                        break;
                    case "1001":
                        userText = msg.getStringFieldValue(Constants_Fields.TAGUserStatusText);
                        Enclosing_Instance.possiblyNotifyListeners(this, FIXTNotice.FIX_CANCEL_DUP_SESSION, userText, msg);
                        break;
                    case "1002":
                        userText = msg.getStringFieldValue(Constants_Fields.TAGUserStatusText);
                        Enclosing_Instance.possiblyNotifyListeners(this, FIXTNotice.FIX_PASS_CHANGE_FAILED, userText, msg);
                        break;                   
                }
            }
            ...
            protected internal virtual bool logoutIfRejectResend(Message msg)
            {
                ...
                System.Int32 refSeqNum = msg.getIntegerValue(Icap.com.scalper.fix.Constants_Fields.TAGRefSeqNum_i);
...
//                if ((refSeqNum != null) && Enclosing_Instance.resendSeqs.Contains(refSeqNum))
        ...
        private class FIXReaderThread : SupportClass.ThreadClass
        {
            ...
            internal sbyte[] delimiterBytes = new sbyte[] { (sbyte)Icap.com.scalper.fix.FixConstants_Fields.delimiter };
                ...
                Run()
            {
                ...
                try
                {
                    ...
                    while (keepRunning)
                    {
                        ...
                        try
                        {
                            ...
                            messageParsed = true;
                            if (parsedMsg.MsgType != "X" && parsedMsg.MsgType != "0")
                            {
                ...
                catch (DenialOfServiceException dos)
                {
                    ...
                    System.String patternString = "8=FIX[.]4[.].^9=([\\d]+)(^.*^)10=[\\d]{3}^";
                    patternString = patternString.Replace('^', Icap.com.scalper.fix.FixConstants_Fields.delimiter);
                ...
                catch (System.IO.IOException e)
                {
                    ...
                    switch (sessionStateInt)
                    {
                        ...
                        default:
                            ...
                            Enclosing_Instance.possiblyNotifyListeners(this, FIXTNotice.FIX_CONNECTION_FAILED, "Counterparty is dead, session will shut down.");
                            break;
            ...
            private void printMessageReceived(Message msg)
            {
                if ((Enclosing_Instance.PrintFixBefore || Enclosing_Instance.PrintFixAfter) && (true))
                {
                    ...
                    buf.Append(",35=");
                    buf.Append(msg.getValue(Icap.com.scalper.fix.Constants_Fields.TAGMsgType_i));
                    buf.Append(" ] ");
            ...
            {
                // in testing mode it's sometimes useful to force the reader thread to sleep in order
                // to enforce a certain sequence of messages
                //maybeSleep();//giriIcap
                ...
                {
                    System.String text = parsedMsg.getValue(Icap.com.scalper.fix.Constants_Fields.TAGText_i);
                    if (text == null || text.Equals(""))
                    {
                        text = "<no message available>";
                    }

                    Enclosing_Instance.possiblyNotifyListeners(this, FIXTNotice.FIX_LOGON_FAILED, "Server refused login attempt: " + text);
                ...
                if (msgType == '1')
                {
                    Enclosing_Instance.timer.resetHeartbeatTimer();
                    Message copy = parsedMsg.Copy;
                    string testRequestID = copy.getStringFieldValue(Icap.com.scalper.fix.Constants_Fields.TAGTestReqID_i);
                    Message testRequestResponse = Enclosing_Instance.buildHeartBeatMessage(testRequestID);
                    Enclosing_Instance.transmitInternal(testRequestResponse);
                    Enclosing_Instance.nextExpectedSeqNo++;
                    return;
                }
                ...
                if (sessionStateInt == SessionState.WAITING_FOR_LOGON || sessionStateInt == SessionState.LOGON_SENT)
                {
                    if (msgType != 'A')
                    {
                        ...
                        Message logoutMsg = Enclosing_Instance.buildLogoutMessage();
                        Enclosing_Instance.OptionalTags = logoutMsg;
                        logoutMsg.setValue(Icap.com.scalper.fix.Constants_Fields.TAGText_i, errorText);
                        //Send logout
                        Enclosing_Instance.transmitInternal(logoutMsg);
                        Enclosing_Instance.nextExpectedSeqNo++;
                        // @todo should drop connection right away
            ...
            protected internal virtual void processSequenceReset_GapFill(Message msg)
            {
                ...
                int newSeqNo = msg.getIntValue(Icap.com.scalper.fix.Constants_Fields.TAGNewSeqNo_i);
        ...
        public virtual void checkStoredMsg(Message msg)
        {
            ...
            char mVersion = msg.getValue(Icap.com.scalper.fix.Constants_Fields.TAGBeginString_i)[6];
            if (minorVersion != mVersion)
            {
                //Version mismatch in the middle of the session!!
                possiblyNotifyListeners(this, FIXTNotice.FIX_VERSION_MISMATCH, "Version mismatch");
        ...
        {
            if (msg.MsgType.Equals(Icap.com.scalper.fix.Constants_Fields.MSGReject))
            {
        ...
        private void transmitMsgSeqNumTooLow(Message msg)
        {
            ...
            try
            {
                ...
                logoutMsg.setValue(Icap.com.scalper.fix.Constants_Fields.TAGText_i, "MsgSeqNum too low, expecting " + nextExpectedSeqNo + " but received " + seqNo);
        ...
        {
            if (!ScalperFixSession.isAdmin(message.getCharValue(Icap.com.scalper.fix.Constants_Fields.TAGMsgType)))
                message.setValue(Icap.com.scalper.fix.Constants_Fields.TAGPossDupFlag_i, "Y");
            ...
            message.removeTag(Icap.com.scalper.fix.Constants_Fields.TAGBodyLength_i);
            message.removeTag(Icap.com.scalper.fix.Constants_Fields.TAGCheckSum_i);
            ...
            lock (transmitLock)
            {
                ...
                if (!setOrigSendingTime)
                {
                    if (!(message.MsgSeqNum < outMsgSeqNo || sessionState.thisSessionMustResend()) && persistFlag)
                    {
                        ...
                        {
                            possiblyNotifyListeners(this, FIXTNotice.FIX_COULDNT_PERSIST_OUTBOUND_MESSAGE, "Couldn't persist message " + message);
                            throw fe;
                        }
                    }
                    else if ((message.MsgType == null) || !message.MsgType.Equals(Icap.com.scalper.fix.Constants_Fields.MSGSequenceReset))
                    {
                        if (testMode)
                        {
        ...
        private void _transmitInternal(System.Object[] msgs, int offset, int count, bool setOrigSendingTime, bool checkIfResending, bool messageIsFromDriver)
        {
            ...
            lock (transmitLock)
            {
                ...
                if (!setOrigSendingTime)
                {
                    ...
                    {
                        possiblyNotifyListeners(this, FIXTNotice.FIX_COULDNT_PERSIST_OUTBOUND_MESSAGE, "Couldn't persist message group");
                        throw fe;
                    }
                }
        ...
        private void preprocessBeforeSend1(Message message, bool setOrigSendingTime, bool messageIsFromDriver)
        {
            ...
            if (PrintFixBefore)
            {
                ...
                buf.Append(",35=");
                buf.Append(message.getValue(Icap.com.scalper.fix.Constants_Fields.TAGMsgType_i));
                buf.Append(" ]");
            ...
            if (testMode)
            {
                message.setIfNull(Icap.com.scalper.fix.Constants_Fields.TAGBeginString_i, BeginFixVersion);
            }
            else
            {
                message.setValue(Icap.com.scalper.fix.Constants_Fields.TAGBeginString_i, BeginFixVersion);
            }
            ...
            else if (sessionStateInt != SessionState.WAITING_FOR_MESSAGE_SYNCH && sessionStateInt != SessionState.SESSION_READY && sessionStateInt != SessionState.LOGOUT_RECEIVED && sessionStateInt != SessionState.LOGON_RESP_SENT)
            {
                ...
                if (msgType != 'A' && !testMode)
                {
                    ...
                    else if (sessionStateInt == SessionState.LOGOUT_SENT && messageIsFromDriver && ((msgType == '4') || (message.getCharValue(Icap.com.scalper.fix.Constants_Fields.TAGPossDupFlag_i) == 'Y')))
                    {
                ...
                else if (msgType == 'A')
                {
                    ...
                    System.String beginString = message.getValue(Icap.com.scalper.fix.Constants_Fields.TAGBeginString_i);
                    if (beginString == null)
                    {
                        missing = true;
                    ...
                    if (!missing)
                    {
                        SenderCompID = message.getValue(Icap.com.scalper.fix.Constants_Fields.TAGSenderCompID_i);
                        if (senderCompID == null)
                        {
                            missing = true;
                    ...
                    if (!missing)
                    {
                        TargetCompID = message.getValue(Icap.com.scalper.fix.Constants_Fields.TAGTargetCompID_i);
                        if (targetCompID == null)
                        {
                            missing = true;
                    ...
                    if (!missing)
                    {
                        int hbInt = message.getIntValue(Icap.com.scalper.fix.Constants_Fields.TAGHeartBtInt_i);
                        if (hbInt == -1)
                        {
                            missing = true;
                        ...
                        else
                        {
                            ...
                            timer.TestRequestInterval = (hbInt * 10) + hbInt / 5;//giri
                        }
                    }
                    if (missing)
                    {
                        log.Error("FFillFixSession.transmit(): Required field [" + fieldName + "]  missing in logon message");
                        FIXTNotice fieldMissing = new FIXTNotice(this, message, this, FIXTNotice.FIX_FIELD_MISSING, fieldName + " is null!");
                        if ((listener != null) && listenerInterests[FIXTNotice.FIX_FIELD_MISSING])
                        {
                    ...
                    else
                    {
                        ...
                        message.setValue(Icap.com.scalper.fix.Constants_Fields.TAGHeartBtInt_i, timer.HBeatInterval / 1000);
        ...
        private bool preprocessBeforeSend2(Message message, bool setOrigSendingTime, bool messageIsFromDriver)
        {
            ...
            if (!testMode || BasicUtilities.isNullOrEmpty(message.getSendingTime()) || setOrigSendingTime)
            {
                ...
                message.setValue(Icap.com.scalper.fix.Constants_Fields.TAGSendingTime_i, sendingTime);
            ...
            if (setOrigSendingTime)
            {
                ...
                message.setValue(Icap.com.scalper.fix.Constants_Fields.TAGOrigSendingTime_i, origSendingTime);
            }
            ...
            if (!message.containsTag(Icap.com.scalper.fix.Constants_Fields.TAGBodyLength_i))
                message.insertValueAt(Icap.com.scalper.fix.Constants_Fields.TAGBodyLength_i, "", 1);

            if (!message.containsTag(Icap.com.scalper.fix.Constants_Fields.TAGCheckSum_i))
                message.addValue(Icap.com.scalper.fix.Constants_Fields.TAGCheckSum, "");
        ...
        private void postprocessBeforeSend1(Message message)
        {
            ...
            */
            ...
            try
            {
                ...
                if (testMode && message.getCharValue(Icap.com.scalper.fix.FixConstants_Fields.TAGDropMessage) == 'Y' && !ScalperFixSession.isPossDup(message))
                {
        ...
        protected internal virtual void stopImmediatelyImpl()
        {
            ...
            possiblyNotifyListeners(this, FIXTNotice.FIX_DISCONNECTED, "Disconnected");
        ...
        protected internal virtual void stopInboundImpl()
        {
            //Set the session state.
        ...
        private void doValidate(Message message)
        {
            ...
            System.String sender = message.getValue(Icap.com.scalper.fix.Constants_Fields.TAGSenderCompID_i);
            if (sender == null)
            {
                ...
                fMfe.RefTagID = Icap.com.scalper.fix.Constants_Fields.TAGSenderCompID_i;
                throw fMfe;
            }
            ...
            System.String target = message.getValue(Icap.com.scalper.fix.Constants_Fields.TAGTargetCompID_i);
            if (target == null)
            {
                ...
                fMfe.RefTagID = Icap.com.scalper.fix.Constants_Fields.TAGTargetCompID_i;
                throw fMfe;
            }
        ...
        private void checkFieldsForMsgType(Message message)
        {
            ...
            if (msgType.Equals(Icap.com.scalper.fix.Constants_Fields.MSGLogon))
            {
                // Logon message
                ...
                try
                {
                    System.String encryptMethodStr = message.getValue(Icap.com.scalper.fix.Constants_Fields.TAGEncryptMethod_i);
                    //int encryptMethod=0;
                    ...
                    System.String heartBtIntStr = message.getValue(Icap.com.scalper.fix.Constants_Fields.TAGHeartBtInt_i);
            ...
            else if (msgType.Equals(Icap.com.scalper.fix.Constants_Fields.MSGTestRequest))
            {
                ...
                try
                {
                    System.String testReqID = message.getValue(Icap.com.scalper.fix.Constants_Fields.TAGTestReqID_i);
                    if (testReqID == null)
            ...
            else if (msgType.Equals(Icap.com.scalper.fix.Constants_Fields.MSGResendRequest))
            {
                ...
                try
                {
                    System.String beginSeqNoStr = message.getValue(Icap.com.scalper.fix.Constants_Fields.TAGBeginSeqNo_i);
                    if (beginSeqNoStr != null)
                        System.Int32.Parse(beginSeqNoStr);
                    else
                        throw new System.NullReferenceException("BeginSeqNo field is missing.");

                    System.String endSeqNoStr = message.getValue(Icap.com.scalper.fix.Constants_Fields.TAGEndSeqNo_i);
                    if (endSeqNoStr != null)
            ...
            else if (msgType.Equals(Icap.com.scalper.fix.Constants_Fields.MSGReject))
            {
                ...
                try
                {
                    System.String refSeqNumStr = message.getValue(Icap.com.scalper.fix.Constants_Fields.TAGRefSeqNum_i);
            ...
            else if (msgType.Equals(Icap.com.scalper.fix.Constants_Fields.MSGSequenceReset))
            {
                ...
                try
                {
                    System.String newSeqNoStr = message.getValue(Icap.com.scalper.fix.Constants_Fields.TAGNewSeqNo_i);
                    if (newSeqNoStr != null)
        ...
        {
            if (newFixVersion.StartsWith("FIXT."))//giriIcap
            {
                newFixVersion = newFixVersion.Substring("FIXT.".Length);//giriIcap
            }
            ...
            switch (minorVersionTemp)
            {
                ...
                case '1': tagHelper = new TagHelper("FIX.4.1"); break;//giriIcap

                case '2': tagHelper = new TagHelper("FIX.4.2");  //<-giri
                    break;
                ...
                case '5': tagHelper = new TagHelper("FIXT.1.1"); break;
            ...
            this.beginFixVersion = "FIXT." + fixVersion;//giriIcap
            ...
            if (minorVersion >= '2')
            {
                resendInfinity = Icap.com.scalper.fix.Constants_Fields.RESEND_INFINITY_42;
            }
            else
            {
                resendInfinity = Icap.com.scalper.fix.Constants_Fields.RESEND_INFINITY_40_41;
            }
        }
        ...
        {
            char msgType = msg.getCharValue(Icap.com.scalper.fix.Constants_Fields.TAGMsgType_i);
            char gapFillChar = msg.getCharValue(Icap.com.scalper.fix.Constants_Fields.TAGGapFillFlag_i);
        ...
        {
            char possDupeFlag = msg.getCharValue(Icap.com.scalper.fix.Constants_Fields.TAGPossDupFlag_i);
            return possDupeFlag == 'Y';
        }
        private static bool isPossResend(Message msg)
        {
            char possResendFlag = msg.getCharValue(Icap.com.scalper.fix.Constants_Fields.TAGPossResend_i);
        ...
        public class SendCommand
        {
            private void InitBlock(ScalperFixSession enclosingInstance)
            {
                ...
                cmdType = Icap.com.scalper.fix.driver.ScalperFixSession.SendCommand.SEND;
            }
            private ScalperFixSession enclosingInstance;
            public ScalperFixSession Enclosing_Instance
            {
