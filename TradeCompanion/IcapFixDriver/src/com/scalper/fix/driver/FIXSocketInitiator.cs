...
namespace Icap.com.scalper.fix.driver
{
	...
	public class FIXSocketInitiator:FIXInitiator
	{
		...
		public override void  start()
		{
			for (int i = 0; i <= retryCount; i++)
			{
				...
				if (connection != null)
				{
					...
					FIXTNotice notice = new FIXTNotice(this, connection, FIXTNotice.FIX_CONNECTION_ESTABLISHED, "Connection established");
					if (listener != null)
					{
						if (listenerInterests[FIXTNotice.FIX_CONNECTION_ESTABLISHED])
		...
		{
			FIXTNotice notice = new FIXTNotice(this, connection, FIXTNotice.FIX_DISCONNECTED, "Disconnected");
			if (listener != null)
			{
				if (listenerInterests[FIXTNotice.FIX_DISCONNECTED])
					listener.takeNote(notice);
				else
