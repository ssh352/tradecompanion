...
namespace FxIntegral.com.scalper.fix.driver.client
{
	public abstract class DefaultClient:AbstractClient
	{
		...
		{
			public AnonymousClassClientSessionCustomizer(FxIntegral.com.scalper.fix.driver.FIXPersister persister, DefaultClient enclosingInstance)
			{
				InitBlock(persister, enclosingInstance);
			}
			private void  InitBlock(FxIntegral.com.scalper.fix.driver.FIXPersister persister, DefaultClient enclosingInstance)
			{
				this.persister = persister;
				this.enclosingInstance = enclosingInstance;
			}
			private FxIntegral.com.scalper.fix.driver.FIXPersister persister;
        ...
        protected abstract void handleMsgReceived(FxIntegral.com.scalper.fix.Message msg);
        private class DefaultClientMessageListener : FIXMessageListener
        {
            ...
            public virtual void messageReceived(FxIntegral.com.scalper.fix.Message msg)
            {
                 enclosingInstance.handleMsgReceived(msg);
            }
            /// <summary>notification that a message has been sent by the driver </summary>
            public virtual void messageSent(FxIntegral.com.scalper.fix.Message msg)
            {
