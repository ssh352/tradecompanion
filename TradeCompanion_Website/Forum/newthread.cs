...
// newthread OpenAnyPage Event end
		...
		if (!IsPostBack){
			...
			p_messages_message_id.Value = Utility.GetParam("message_id");
            Page_Show(sender, e);
		}
	}
...
// Utility.GetParam("_ctl0:ContentPlaceHolder1:messages_message")
        //string p2_smiley_id = CCUtility.ToSQL(Utility.GetParam("_ctl0:ContentPlaceHolder1:messages_smiley_id"), FieldTypes.Number);
        string p2_topic = CCUtility.ToSQL(Utility.GetParam("_ctl0:ContentPlaceHolder1:messages_topic"), FieldTypes.Text);
        string p2_author = CCUtility.ToSQL(Utility.GetParam("_ctl0:ContentPlaceHolder1:messages_author"), FieldTypes.Text);
        string p2_message = CCUtility.ToSQL(Utility.GetParam("_ctl0:ContentPlaceHolder1:messages_message"), FieldTypes.Memo);
        string p2_date_entered = CCUtility.ToSQL(Utility.GetParam("_ctl0:ContentPlaceHolder1:messages_date_entered"), FieldTypes.Date);
        string p2_last_modified = CCUtility.ToSQL(Utility.GetParam("_ctl0:ContentPlaceHolder1:messages_last_modified"), FieldTypes.Date);
        //p2_smiley_id = CCUtility.ToSQL(Utility.GetParam("_ctl0:ContentPlaceHolder1:smiley_id"), FieldTypes.Number);

        //if (p2_smiley_id == null)
        int p2_smiley_id = 0;
